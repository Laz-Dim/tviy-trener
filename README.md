...
3. **`python generate_blog_posts.py`**: Створює/оновлює файл `blog_posts.json` та окремі файли постів у папці `posts/`. Цей скрипт імітує AI-обробку тексту.

### Покращена обробка (Audio & AI)
Для майбутнього впровадження якісної транскрипції додано:
- **`download_audio.py`**: Скрипт для скачування аудіодоріжок відео. Це дозволяє використовувати Whisper AI для більш точного розпізнавання мови порівняно з субтитрами YouTube.
